...
EVENTS = {
    "SYNC-GOOD": [PARENT / "data" / "create.request.json"],
    "ASYNC-GOOD": [PARENT / "data" / "create.with-request-context.request.json"],
}
...
class TestCloudWatchScheduler(unittest.TestCase):
    def test_reschedule(self):
        ...
        self.assertEqual(
            True,
            event["requestContext"]["cloudWatchEventsRuleName"].startswith(
                "reinvoke-handler-"
            ),
        )
        self.assertEqual(
            True,
            event["requestContext"]["cloudWatchEventsTargetId"].startswith(
                "reinvoke-target-"
            ),
        )
