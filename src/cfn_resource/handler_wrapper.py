...
from .utils import setup_json_logger, get_log_level_from_env, is_sam_local
from .boto3_proxy import get_boto3_proxy_session, get_boto_session_config
from .exceptions import InternalFailure, Codes
...
def _handler_wrapper(event, context):
    ...
    try:
        ...
        progress_event.errorCode = type(e).__name__ if Codes.is_handled(e) else Codes.INTERNAL_FAILURE
...
class HandlerWrapper:
    ...
    def __init__(self, event, context):
        ...
        self._handler_response = ProgressEvent(Status.FAILED, resourceModel=ResourceModel())
        self._session_config = get_boto_session_config(event)
        self._metrics = Metrics(resource_type=event['resourceType'], session_config=self._session_config)
    ...
    def _get_handler(self, handler_path='handlers'):
        ...
        try:
            ...
            raise InternalFailure("handlers.py does not contain a {}_handler function".format(self._action))

    def _event_parse(self):
        resource_properties, previous_resource_properties, callback_context = request.extract_event_data(self._event)
        session_config = get_boto_session_config(self._event)
        kwargs = {
            'resource_model': ResourceModel.new(**resource_properties),
        ...
        if self._event['action'] in [request.Action.CREATE, request.Action.UPDATE, request.Action.DELETE]:
            kwargs['callback_context'] = callback_context
        # Update action gets previous properties
        if self._event['action'] == request.Action.UPDATE:
            kwargs['previous_resource_model'] = ResourceModel.new(**previous_resource_properties)
        return kwargs
    ...
    def run_handler(self):
        try:
            ...
            if self._handler_response.callbackDelayMinutes > 0 and self._handler_response.status == Status.IN_PROGRESS:
            ...
            self._handler_response.errorCode = type(e).__name__ if Codes.is_handled(e) else Codes.INTERNAL_FAILURE
        finally:
            try:
                self._metrics.duration(self._start_time, action=self._action,
                                       duration=datetime.now() - self._start_time)
                if is_sam_local():
                    LOG.warning("Skipping publishing CloudWatch metrics as invocation is SAM local")
                else:
