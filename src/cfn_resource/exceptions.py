class Codes:
    ...
    # the request was unable to be completed due to networking issues, such as failure to receive a response from the
    # server. Handlers SHOULD retry on network failures using exponential backoff in order to be resilient to transient
    # issues.
    ...
    NOT_READY = 'NotReady'
    # the request was throttled by the downstream service. Handlers SHOULD retry on service throttling using
    # exponential backoff in order to be resilient to transient throttling.
    ...
    def is_handled(cls, e: Exception):
        if type(e).__name__ in [v for k, v in Codes.__dict__.items() if type(v) is str and not v.startswith("_")]: # pylint: disable=unidiomatic-typecheck
            return True
        return False
